...
export default function AddRoomModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validation
    if (!formData.id.trim()) {
      newErrors.id = 'กรุณากรอก ID ห้อง'
    }
    ...
    try {
      ...
      if (error) {
        ...
        Swal.fire({
          icon: 'error',
          title: 'เกิดข้อผิดพลาด',
          text: error.message || 'ไม่สามารถเพิ่มห้องได้ กรุณาลองใหม่อีกครั้ง',
          confirmButtonText: 'ตกลง',
          confirmButtonColor: '#0d9488',
        })
        return
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div>
              <Input
                ...
                error={errors.id}
                placeholder="เช่น deluxe-city"
                helperText="ใช้ตัวพิมพ์เล็กและขีดกลาง (เช่น deluxe-city)"
              />
            </div>
