...
export default function LoginPage() {
  ...
  const [showTestPassword, setShowTestPassword] = useState({ admin: false, member: false })
    ...
    <div className="min-h-screen flex items-center justify-center py-12 px-4 bg-gradient-to-b from-teal-50 via-blue-50 to-pink-50">
      <Container>
        <div className="flex flex-col lg:flex-row gap-6 items-start justify-center max-w-5xl mx-auto">
          {/* Login Card */}
          <Card className="w-full lg:w-auto lg:max-w-md flex-shrink-0 p-8 shadow-xl">
            {/* Back to Home Link */}
            <div className="mb-6">
              <Link
                to="/"
          ...
          <Card className="w-full lg:w-auto lg:max-w-md flex-shrink-0 p-6 shadow-xl border-2 border-amber-200 bg-amber-50/50">
            <div className="mb-4">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1">
                      <span className="font-mono text-xs text-slate-800">
                        {showTestPassword.member ? 'member1234' : '••••••••'}
                      </span>
                      <button
                        onClick={() => setShowTestPassword(prev => ({ ...prev, member: !prev.member }))}
                        className="text-slate-400 hover:text-teal-600 transition-colors"
                        title={showTestPassword.member ? 'ซ่อนรหัสผ่าน' : 'แสดงรหัสผ่าน'}
                      >
                        {showTestPassword.member ? <EyeOff size={14} /> : <Eye size={14} />}
                      </button>
                      <button
