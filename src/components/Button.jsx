...
export default function Button({
  ...
  const disabledClass = disabled
    ? 'opacity-50 cursor-not-allowed bg-slate-400 hover:bg-slate-400 hover:shadow-none'
    : ''

  return (
    <button
      className={`inline-flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-sm font-semibold ${variantClass} ${disabledClass} ${className}`}
      disabled={disabled}
      {...props}
    >
      {children}
